...
const topicadder = () => {
  data.Topics.forEach((topic) => {
    if (topic["Topic Level 1"]) {
      ...
      let topi = `${topic["Topic Level 1"]}`;
      let path = "null";
      //push to mongodb databse
      addEntry({ topic: topi, path: path });
    }

    if (topic["Topic Level 2"]) {
      let topi = `${topic["Topic Level 2"]}`;
      let path = `,${topic["Topic Level 1"]},`;
      addEntry({ topic: topi, path: path });
    }
    if (topic["Topic Level 3"]) {
      let topi = `${topic["Topic Level 3"]}`;
      let path = `,${topic["Topic Level 1"]},${topic["Topic Level 2"]},`;
      addEntry({ topic: topi, path: path });
    }
  });
};
...
const questionadder = () => {
  data.Questions.forEach((quest) => {
    ...
    if (quest["Annotation 1"]) {
      getId(quest["Annotation 1"])
        .then((id) => {
          annot.push(id);

          createQuestion(
            { questionno: quest["Question number"] },
            { questionno: quest["Question number"], annotations: annot }
          );
        })
        .catch((err) => {});
    }
    if (quest["Annotation 2"]) {
      getId(quest["Annotation 2"])
        .then((id) => {
          annot.push(id);
          console.log(annot);
          console.log({
    ...
    if (quest["Annotation 3"]) {
      getId(quest["Annotation 3"])
        .then((id) => {
          annot.push(id);
          createQuestion(
            { questionno: quest["Question number"] },
    ...
    if (quest["Annotation 4"]) {
      getId(quest["Annotation 4"])
        .then((id) => {
          annot.push(id);
          createQuestion(
            { questionno: quest["Question number"] },
            { questionno: quest["Question number"], annotation: annot }
          );
        })
        .catch((err) => {});
    }
    if (quest["Annotation 5"]) {
      getId(quest["Annotation 5"])
        .then((id) => {
          annot.push(id);
          createQuestion(
            { questionno: quest["Question number"] },
